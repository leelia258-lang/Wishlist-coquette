...
function updateThemeColorMeta() {
    const colors = { 
        blush: { bg: '#FDE2E7', theme: '#F8C8DC' },
        lavender: { bg: '#E8DFF5', theme: '#C3AED6' },
        sakura: { bg: '#FDE2E7', theme: '#F4A7B9' },
        matcha: { bg: '#E8F0E3', theme: '#A8C89A' },
        blue: { bg: '#E0ECF5', theme: '#A8C8E8' }
    };
    
    const metaTheme = document.getElementById('metaThemeColor');
    const metaBg = document.getElementById('metaBgColor');
    
    const colorSet = isDarkMode 
        ? { bg: '#2D1F2C', theme: '#4A2F45' } 
        : (colors[currentTheme] || colors.blush);
    
    if (metaTheme) metaTheme.content = colorSet.theme;
    if (metaBg) metaBg.content = colorSet.bg;
    
    // Update manifest theme_color secara dinamis
    updateManifestColors(colorSet);
}

function updateManifestColors(colorSet) {
