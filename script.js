...
const translations = {
    id: {
        ...
        label_name: 'Nama Produk',
        label_category: 'Kategori',
        label_link: 'Link Produk',
        label_image: 'Gambar Produk (URL)',
        label_price: 'Harga (Rp)',
        label_store_detected: 'Toko Terdeteksi',
        placeholder_name: 'Contoh: Dress vintage putih...',
        placeholder_link: 'Masukkan link produk...',
        placeholder_image: 'https://...',
        placeholder_price: '150000',
        placeholder_search: 'Cari produk...',
        placeholder_cat_name: 'Nama kategori...',
        option_no_category: 'Tanpa Kategori',
        btn_add: 'Tambah ke Wishlist',
        btn_cancel: 'Batal',
        btn_save: 'Simpan',
        btn_show_all: 'Tampilkan Semua',
        btn_add_first: 'Tambah Sekarang',
        btn_add_category: 'Tambah Kategori',
        wishlist_count_1: 'Kamu punya',
        wishlist_count_2: 'item di wishlist',
        ...
        sort_az: 'Nama A-Z',
        sort_za: 'Nama Z-A',
        no_result_title: 'Produk tidak ditemukan',
        no_result_text: 'Coba kata kunci lain!',
        empty_title: 'Wishlist kamu masih kosong',
        empty_text: 'Yuk, tambahkan produk impianmu dulu!',
        cat_form_title: 'Tambah Kategori Baru',
        cat_list_title: 'Daftar Kategori',
        cat_empty_title: 'Belum ada kategori',
        cat_empty_text: 'Buat kategori pertamamu di atas!',
        cat_name_label: 'Nama Kategori',
        modal_edit_title: 'Edit Wishlist',
        modal_edit_cat_title: 'Edit Kategori',
        toast_added: 'Berhasil ditambahkan! 🎀',
        toast_updated: 'Wishlist diperbarui! ✨',
        toast_deleted: 'Item dihapus 💔',
        toast_cat_added: 'Kategori ditambahkan! 🏷️',
        toast_cat_updated: 'Kategori diperbarui! ✨',
        toast_cat_deleted: 'Kategori dihapus! 🗑️',
        ...
        label_purchased: 'Dibeli',
        label_not_purchased: 'Belum dibeli',
        label_no_link: 'Tanpa link',
        label_open_link: 'Buka Link',
        label_no_name: 'Tanpa Nama',
    ...
    en: {
        ...
        label_link: 'Product Link',
        label_image: 'Product Image (URL)',
        label_price: 'Price (Rp)',
        label_store_detected: 'Store Detected',
        placeholder_name: 'E.g.: Vintage white dress...',
        placeholder_link: 'Enter product link...',
        placeholder_image: 'https://...',
        placeholder_price: '150000',
        placeholder_search: 'Search products...',
        placeholder_cat_name: 'Category name...',
        option_no_category: 'No Category',
        btn_add: 'Add to Wishlist',
        btn_cancel: 'Cancel',
        btn_save: 'Save',
        btn_show_all: 'Show All',
        btn_add_first: 'Add Now',
        btn_add_category: 'Add Category',
        wishlist_count_1: 'You have',
        wishlist_count_2: 'items in wishlist',
        total_label: 'Total Wishlist',
        ...
        sort_az: 'Name A-Z',
        sort_za: 'Name Z-A',
        no_result_title: 'Product not found',
        no_result_text: 'Try another keyword!',
        empty_title: 'Your wishlist is empty',
        empty_text: 'Add your dream products first!',
        cat_form_title: 'Add New Category',
        cat_list_title: 'Category List',
        cat_empty_title: 'No categories yet',
        cat_empty_text: 'Create your first category above!',
        cat_name_label: 'Category Name',
        modal_edit_title: 'Edit Wishlist',
        modal_edit_cat_title: 'Edit Category',
        toast_added: 'Successfully added! 🎀',
        toast_updated: 'Wishlist updated! ✨',
        toast_deleted: 'Item deleted 💔',
        toast_cat_added: 'Category added! 🏷️',
        toast_cat_updated: 'Category updated! ✨',
        toast_cat_deleted: 'Category deleted! 🗑️',
        ...
        label_not_purchased: 'Not purchased',
        label_no_link: 'No link',
...
function applyLanguage() {
    // Update semua elemen dengan data-lang
    ...
    // Update option tanpa kategori di dropdown
...
for (const [id, key] of Object.entries(placeholders)) {
    ...
    // Re-render wishlist & categories
    if (document.getElementById('tabList').classList.contains('active')) {
...
function extractStoreFromUrl(url) {
    ...
    try {
        ...
        for (let i = 0; i < parts.length; i++) {
            if (!TLDs.includes(parts[i])) { storeName = parts[i]; break; }
        }
        if (!storeName) storeName = parts[0];
        storeName = storeName.charAt(0).toUpperCase() + storeName.slice(1);
        return { name: storeName, domain: storeName.toLowerCase() };
    } catch (e) { return null; }
}
...
function switchTab(tabName) {
    ...
    if (tabName === 'add') {
        ...
        populateCategoryFilter(); populateStoreFilter(); clearSearch(); renderWishlist();
...
function updateBadge() {
    document.getElementById('badgeCount').textContent = wishlist.length;
    document.getElementById('totalItems').textContent = wishlist.length;
}

// ========== TOTAL HARGA ==========
function updateTotalPrice() {
    const card = document.getElementById('totalPriceCard');
    const amount = document.getElementById('totalPriceAmount');
    if (wishlist.length === 0) { card.style.display = 'none'; return; }
    card.style.display = 'flex';
    let total = 0;
    wishlist.forEach(item => {
        const price = parseInt(String(item.price).replace(/[^0-9]/g, ''));
...
function updateStoreTotals() {
    ...
    const totals = {};
    wishlist.forEach(item => {
        const storeName = item.storeName || 'Tanpa Toko';
        if (!totals[storeName]) totals[storeName] = { total: 0, color: item.storeColor || '#888' };
        const price = parseInt(String(item.price).replace(/[^0-9]/g, ''));
        if (!isNaN(price)) totals[storeName].total += price;
    });
    container.innerHTML = '';
    container.style.display = 'flex';
    for (const [storeName, data] of Object.entries(totals)) {
        const el = document.createElement('span');
        el.className = 'store-total-item';
        el.style.background = data.color;
        el.innerHTML = `<i class="fa-solid fa-store"></i> ${t('store_total_format', { name: storeName, total: data.total.toLocaleString('id-ID') })}`;
...
function populateStoreFilter() {
    ...
    storeNames.forEach(name => { sel.innerHTML += `<option value="${name}">${name}</option>`; });
}
...
function addCategory() {
    ...
    if (!name) { shakeElement(nameInput); return; }
    categories.push({ id: 'cat_' + Date.now(), name });
    saveCategories(); nameInput.value = ''; renderCategoryList();
...
function closeEditCategoryModal() {
    ...
    document.body.classList.remove('modal-open'); currentEditCategoryId = null;
}
function saveEditCategory() {
    if (!currentEditCategoryId) return;
    const name = document.getElementById('editCategoryName').value.trim();
    if (!name) return;
    const cat = categories.find(c => c.id === currentEditCategoryId);
    if (cat) cat.name = name;
    saveCategories(); closeEditCategoryModal(); renderCategoryList();
    populateCategoryDropdowns(); populateCategoryFilter(); renderWishlist();
...
function renderWishlist() {
    ...
    if (wishlist.length === 0) {
        ...
        updateTotalPrice(); updateStoreTotals(); populateStoreFilter(); return;
    }
...
function togglePurchased(index) {
    wishlist[index].purchased = !wishlist[index].purchased;
...
function filterWishlist() {
    ...
    document.querySelectorAll('.wishlist-item').forEach((item, idx) => {
        ...
        if (show) { item.classList.remove('hidden-item'); visibleCount++; }
...
function closeEditModal() {
    ...
    document.getElementById('modalBackdrop').classList.remove('show');
    document.body.classList.remove('modal-open'); currentEditIndex = -1;
}
...
function detectStorePreview() {
    ...
    if (store && link) { badge.style.display = 'inline-flex'; badge.style.background = getStoreColor(store.name); badge.innerHTML = `<i class="fa-solid fa-store"></i> ${store.name}`; name.textContent = ''; }
...
langStyle.textContent = `
    ...
    .lang-btn {
        ...
        background: var(--card-bg); color: var(--accent-text); cursor: pointer;
        font-size: 0.7em; font-weight: 700; display: flex; align-items: center; justify-content: center;
        transition: all 0.3s; font-family: 'Poppins', sans-serif;
    }
    .lang-btn:hover { border-color: var(--accent); }
    .lang-btn.active { background: var(--accent); color: white; border-color: var(--accent-dark); }
`;
